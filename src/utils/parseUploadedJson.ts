...
function parseRegion(raw: Record<string, unknown>, index: number): TextRegion {
  ...
  return {
    ...
    ...(raw['fontWeight'] === 'normal' || raw['fontWeight'] === 'bold' ? { fontWeight: raw['fontWeight'] } : {}),
    ...(raw['fontStyle'] === 'normal' || raw['fontStyle'] === 'italic' ? { fontStyle: raw['fontStyle'] } : {}),
    ...(raw['textDecoration'] === 'none' || raw['textDecoration'] === 'line-through' ? { textDecoration: raw['textDecoration'] } : {}),
